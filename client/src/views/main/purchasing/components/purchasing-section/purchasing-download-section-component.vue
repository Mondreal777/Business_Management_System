        ...
        .input-product-details
            form(method="post")
              ...
              b-field(label="Request Type:") {{ purchaseRequestDetails.requestType }}
                b-input(v-model='purchaseRequestDetails.requestType' id="request_type" size='is-small' hidden)
...
export default {
  name: "PurchasingDownloadComponent",
  components: {
    Header,
    Sidebar,
  },
  ...
  methods: {
    async setPurchaseRequestData() {
      ...
      this.saveAuditTrail(loginDetails["loginUserId"], 'Purchase Request', 'Download purchase requests', null, null); 
      
      window.html2canvas = html2canvas;
      var doc = new jsPDF("p","pt","a4");
      doc.html(document.querySelector(".content-container"), {
        callback: function(pdf) {
        var pdfDate = moment().format('MM-DD-YYYY');
        pdf.save(document.getElementById("request_type").value + "-" +pdfDate + ".pdf");
        window.close();
        }
      });
