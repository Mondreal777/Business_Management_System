    ...
    main
      .content-container
        .menu-container
          ...
          .addButton
            b-button(size="is-small" icon-right="plus-box-multiple" type="is-success" @click="navigateToAddItem") Add Discount
        .table
          b-table(
            :data='filterData' 
            :per-page="perPage" 
            ...
            default-sort="discount_name"
            :range-before= '1'
            :range-after='1'
            )
            b-table-column(field="discount_name" label="Name" v-slot="props" centered sortable)
              | {{ props.row.discount_name }}
            ...
            b-table-column(field="ordering_system" label="Ordering Type" v-slot="props" centered sortable)
              | {{ props.row.ordering_system_name }}
            ...
            b-table-column(field="action" label="Action" width="50" v-slot="props" centered)
              .actionButtons
                .editButton
                  b-button(size="is-small" icon-right="circle-edit-outline" type="is-warning" alt="Edit" @click="navigateToEditItem(props.row.discount_id)")
                .deleteButton
                  b-input(v-model='toBeDeletedId' size='is-small' type='hidden' required)
                  b-button(size="is-small" icon-right="delete-outline" type="is-danger" v-on:click="addToBeDeletedInModal(props.row.discount_id)")
  ...
  b-modal.deleteModal(v-model='isDeleteModalActive' 
            ...
            .modal-card(style='width: auto')
              ...
              section.modal-card-body
                ...
                b-field(label='Are you sure you want to delete this Discount?')
...
export default {
  name: "discountsSectionComponent",
  components: {
    Header,
    Sidebar
  },
  ...
  methods: {
    ...
    async handleSwitch(item) {
        ...
        await axios.post(
            `${process.env.VUE_APP_API_BASE_URL}/discount/update-status/${item.discount_id}`,
            {
                status
            }
        );
        this.$toast.open({
            message: "Discount status is updated!",
            type: "success",
            duration: 5000,
    ...
    navigateToAddItem() {
      this.$router.push('/store-management/discounts/add');
    },
    navigateToEditItem(id) {
      this.$store.commit("setDiscountAndSurchargesId", id);
      this.$router.push('/store-management/discounts/edit/');
    },
    ...
    async deleteItem(){
      ...
      await axios.delete(`${process.env.VUE_APP_API_BASE_URL}/discount/delete/` + this.toBeDeletedId);
      ...
      this.saveAuditTrail(loginDetails["loginUserId"], 'Discount', 'Delete discount', null, null);
    }
  },
  async created() {
    try {
    ...
    filterData: function () {
      ...
      if (!search) {
        ...
        data = data.filter(function(item) {
          if(item.discount_name.toLowerCase().indexOf(search) !== -1 || item.discount_name.indexOf(search) !== -1 || 
            item.deduction === search ||
            item.deduction_type.toLowerCase().indexOf(search) !== -1 || item.deduction_type.indexOf(search) !== -1 ||
            item.ordering_system_name.toLowerCase().indexOf(search) !== -1 || item.ordering_system_name.indexOf(search) !== -1 || 
